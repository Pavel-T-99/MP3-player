...
def get_info(current_id):
    try:
        ...
        if file_type == ".mp3":
            ...
            track_length = FLAC(file_path).info.length
        elif file_type == ".wav":
            track_length = WAVE(file_path).info.length
        else:
...
def add_file_to_playlist():
    global playlist
    global current_id
    filepath = filedialog.askopenfilename(title = "Select File", filetypes = (("MP3 files","*.mp3"),("FLAC files", "*.flac"), ("WAV files", "*.wav")))
    if filepath:
...
def add_dir_to_playlist():
    ...
    if directory:
        ...
        for file in files:
            filetype = file[-4:]
            if (filetype == ".mp3" or filetype == ".wav" or filetype == ".flac"):
                filename = file[:-4]
                playlist_box.insert(len(playlist), filename)
...
def save_playlist():
    global playlist
    if len(playlist) > 0:
        playlist_name = filedialog.asksaveasfilename(title = "Save Playlist", filetypes = (("sapa playlist","*.mldpl"),))
        if playlist_name:
...
def load_playlist():
    global playlist
    global current_id
    playlist_file = filedialog.askopenfilename(title = "Select Playlist", filetypes = (("SaPa playlist","*.mldpl"),))
    if playlist_file:
...
playlist_frame.grid(row = 1, column = 1, padx = 20)
...
playlist_box = Listbox(playlist_box_frame, yscrollcommand=scrollbar.set, selectmode=EXTENDED, width = 50)
...
# status bar
status_bar = ttk.Label(root, text = "Enjoy your music!", relief = RIDGE, anchor = W, font = "Arial 10")
